...
learn_rule <- function(y.in.rct, x.in.rct, a.in.rct, x.in.rwe, x, N, y.star.hat.in.rct, 
                       w.method, method, prob.one, only.weight=F, misspecify=F, iter=30) {
  ## w.method: 1,2,3,4
  ...
  if (w.method == 1) {
    ...
    if (misspecify) {
      ...
      opt2 <- optim(rep(0,p+1), cost2, x.in.rct=x.in.rct**2, x.in.rwe=x.in.rwe**2, N=N, method = "Nelder-Mead", control = list(maxit=10000))
      ...
      opt2 <- optim(rep(0,p+1), cost2, x.in.rct=x.in.rct, x.in.rwe=x.in.rwe, N=N, method = "Nelder-Mead", control = list(maxit=10000))
      (alphas.hat2 <- opt2$par)
    ...
    if (misspecify) {
      ...
      opt <- optim(rep(0, p+1), cost, x.in.rct=x.in.rct**2, left.hand=left.hand, N=N, method = "Nelder-Mead", control = list(maxit=10000))
      (alphas.hat3 <- opt$par)
      ...
      opt <- optim(rep(0, p+1), cost, x.in.rct=x.in.rct, left.hand=left.hand, N=N, method = "Nelder-Mead", control = list(maxit=10000))
      (alphas.hat3 <- opt$par)
  ...
  if (only.weight) {
    ...
    if (method == "linear.LS") {
      ...
      for (it in 1:iter) { ## increase the chance to find the global min
...
learn_rule_given_w.tilde <- function(y.in.rct, x.in.rct, a.in.rct, y.star.hat.in.rct, 
                                     x, method, prob.one, w.tilde, iter=30) {
  ## w.method: 1,2,3,4
  ## method: "linear.LS", "linear.VS", "tree"
  n <- dim(x.in.rct)[1]
  p <- dim(x.in.rct)[2] - 1
  ...
  return(list(action=action, beta.hat.star=beta.hat.star, value=val))
}

value_est <- function(y, x, a, actions, w.tilde.normalized, ipw, weighted.para=F, family="gaussian") {
  ## actions: actions given by the evaluation policy
  ## a: actions given by the behavior policy
  ## x: covariates including intercepts 1's
  C <- (actions==a)
  if (ipw) {
    value <- mean(y*C / ps)
  } else {
    ...
    if (weighted.para)  lm_fit.out <- glm(y ~ x + a*x-1-a, weights=w.tilde.normalized, family=family) else
      lm_fit.out <- glm(y ~ x + a*x-1-a, family = family)
    betas <- lm_fit.out$coefficients
    Q <- cbind(x, actions*x)%*% as.matrix(betas, length(betas),1)
    if (family == "quasibinomial") Q <- 1/(1+exp(-Q))
...
cv.new <- function(y.in.rct, x.in.rct, a.in.rct, x.in.rwe, N, nfold, prob.one,
                   ipw=F, method, baseline, contrast, x, calculate.true.val=F, misspecify=F,
                   weighted.para, family="gaussian", iter=30) {
    ...
    check <- tryCatch(
      { 
        ...
        for (w.method in 4:1) {
          ...
          value <- value_est(y.in.rct[eval.ind], x.in.rct[eval.ind, ], a.in.rct[eval.ind], 
                             as.numeric(as.character(action)), w.tilde.normalized.eval, ipw=ipw, weighted.para=T, family=family)$value
          # value <- value_est(y.in.rct[eval.ind]*w.tilde.normalized, sweep(x.in.rct[eval.ind, ], MARGIN=1, w.tilde.normalized, `*`), 
          #                    a.in.rct[eval.ind], as.numeric(as.character(action)), ps=rep(0.5, n-round(n*prop.train)), ipw=ipw)$value
          if (is.na(value)) {
            # print(w.tilde)
...
cv.real.data.new <- function(y.in.rct, x.in.rct, a.in.rct, x.in.rwe, N=NA, nfold, prob.one=NA,
                             ipw=F, method="linear.VS", misspecify=F, weighted.para=T, family="gaussian", 
                             y.hat.meth, Qm, iter=30) {
  #### consider to use weighted.para in both learn and evaluate the linear rule in the splitted samples ####
  #### x is only needed when calculate.true.val=TRUE ####
  n <- dim(x.in.rct)[1]
  m <- dim(x.in.rwe)[1]
      ...
      { 
        ...
        for (w.method in 4:1) {
          ...
          y.star.hat.in.rct <- est.contrast(a.in.rct[train.ind], x.in.rct[train.ind, ], y.in.rct[train.ind], meth=y.hat.meth, 
                                            psm=1, Qm=Qm, regr.direct=F, weighted.para=weighted.para, w.tilde=w.tilde.train, family=family)
          action <- learn_rule_given_w.tilde(y.in.rct[train.ind], x.in.rct[train.ind, ], a.in.rct[train.ind], y.star.hat.in.rct, 
                                             x.in.rct[eval.ind, ], method, prob.one = NA, w.tilde = w.tilde.train)$action
          Betas <- Variable(p+1)
          f <- x.in.rct[train.ind, ] %*% Betas
          hin <- pos(1-(2*as.numeric(y.star.hat.in.rct>0)-1)*f)
          loss <- sum(hin * abs(y.star.hat.in.rct) * w.tilde.train/sum(w.tilde.train)) 
...
est.contrast <- function(a.in.rct, x.in.rct, y.in.rct, meth=1, psm=1, Qm=1, 
                         regr.direct=F, weighted.para=F, w.tilde=NA, family="gaussian") {
  ...
  if (meth==1) {
    ...
    ## Q(X_i,1) - Q(X_i,0)
    x <- x.in.rct[,-1]
    Q.reg <- Q.est(x, y.in.rct, a.in.rct, Qm, weighted.para=weighted.para, w.tilde=w.tilde, family=family)
    ...
    x <- x.in.rct[,-1]
    n <- length(a.in.rct)
    aipw_i <- v.aipw(x, y.in.rct, a.in.rct, psm, Qm, regr.direct, weighted.para=weighted.para, w.tilde=w.tilde, family=family)$aipw_i
...
v.aipw <- function(x, y, A, psm, Qm, regr.direct, weighted.para, w.tilde, family="gaussian") {
  ...
  n <- length(y)
  Q.reg <- Q.est(x, y, A, Qm, weighted.para, w.tilde, family)
  reg.mu0 <- Q.reg$reg.mu0
  reg.mu1 <- Q.reg$reg.mu1
  if (regr.direct) {
...
Q.est <- function(x, y, A, Qm, weighted.para, w.tilde, family="gaussian") {
  if (Qm==1) {
    ...
    n <- length(y)
    dat2 <- data.frame(cbind(y, x, A,  A*x)); names(dat2)[1] <- "y"
    ## use linear model to est Q in aipw
    if (weighted.para)  lin <- glm(y ~ ., data=dat2, weights=w.tilde, family=family) else
      lin <- glm(y ~ ., data=dat2, family = family)
    if (family == "quasibinomial") {
      newdf <- data.frame(x, rep(0, n), x*0)
      names(newdf) <- names(dat2)[-1]
      reg.mu0 <- predict(lin, newdata = newdf, type="response") ## prediction of probability
      newdf <-  data.frame(x, rep(1, n), x)
      names(newdf) <- names(dat2)[-1]
      reg.mu1 <- predict(lin, newdata = newdf, type="response")
    } else {
      newdf <- data.frame(x, rep(0, n), x*0)
      names(newdf) <- names(dat2)[-1]
